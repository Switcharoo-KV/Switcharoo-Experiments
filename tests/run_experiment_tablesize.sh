...
echo "  Table Size: [1024, 65536]" >> log.txt
...
do  
    ...
    do
        ...
        echo "$(date +'%m-%d-%y-%T') - Cuckoo Table Size ${size} ~ Start Run ${i}" >> log.txt
        ...
        echo "$(date +'%m-%d-%y-%T') - Cuckoo Table Size ${size} ~ End Run ${i}" >> log.txt

        sleep 5
    done
    echo "$(date +'%m-%d-%y-%T') - End Cuckoo Table Size ${size}" >> log.txt
done

mkdir -p $path/$configuration/switcharoo-logs
mkdir -p $path/$configuration/mcast-logs
mkdir -p $path/$configuration/fastclick-logs

echo "Copying $SWITCHAROO_TOFINO_NAME logs in $path/$configuration" >> log.txt
sshpass -p $TOFINO_USER_PASS scp -r $TOFINO_USERNAME@$SWITCHAROO_TOFINO_NAME:$SWITCHAROO_PATH/logs/* $path/$configuration/switcharoo-logs

echo "Deleting logs from $SWITCHAROO_TOFINO_NAME..." >> log.txt
sshpass -p $TOFINO_USER_PASS ssh $TOFINO_USERNAME@$SWITCHAROO_TOFINO_NAME "sudo rm -rf $SWITCHAROO_PATH/logs/*"

echo "Copying $MCAST_TOFINO_NAME logs in $path/$configuration" >> log.txt
sshpass -p $TOFINO_USER_PASS scp -r $TOFINO_USERNAME@$MCAST_TOFINO_NAME:$MULTICAST_PATH/logs/* $path/$configuration/mcast-logs

echo "Deleting logs from $MCAST_TOFINO_NAME..." >> log.txt
sshpass -p $TOFINO_USER_PASS ssh $TOFINO_USERNAME@$MCAST_TOFINO_NAME "sudo rm -rf $MULTICAST_PATH/logs/*"

echo "Copying $GENERATOR_SERVER_NAME logs in $path/$configuration" >> log.txt
sshpass -p $SERVER_USER_PASS scp -r $SERVER_USERNAME@$GENERATOR_SERVER_NAME:$GENERATOR_PATH/logs/* $path/$configuration/fastclick-logs
