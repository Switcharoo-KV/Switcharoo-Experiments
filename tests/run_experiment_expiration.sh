...
for expiration in 50000 500000 5000000 50000000 500000000 1000000000
do  
    ...
    echo "$(date +'%m-%d-%y-%T') - Building switcharoo with expiration ${expiration}" >> log.txt
    echo "$(date +'%m-%d-%y-%T') - Command: ~/tools/p4_build.sh -DTABLE_SIZE=65536 -DENTRY_TIMEOUT=${expiration} $SWITCHAROO_PATH/p4src/switcharoo.p4" >> log.txt
    
    sshpass -p $TOFINO_USER_PASS ssh $TOFINO_USERNAME@$SWITCHAROO_TOFINO_NAME SDE=$SWITCHAROO_TOFINO_SDE SDE_INSTALL=$SWITCHAROO_TOFINO_SDE_INSTALL  "~/tools/p4_build.sh -DTABLE_SIZE=65536 -DENTRY_TIMEOUT=${expiration} $SWITCHAROO_PATH/p4src/switcharoo.p4"
    ...
    do
        ...
        echo "$(date +'%m-%d-%y-%T') - Expiration ${expiration} ~ Start Run ${i}" >> log.txt
        ...
        echo "$(date +'%m-%d-%y-%T') - Expiration ${expiration} ~ End Run ${i}" >> log.txt

        sleep 5
    done
    echo "$(date +'%m-%d-%y-%T') - End Expiration ${expiration}" >> log.txt

    mkdir -p $path/$configuration/$expiration/switcharoo-logs
